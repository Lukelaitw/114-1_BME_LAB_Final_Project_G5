...
class Renderer:
    ...
    def _draw_player(self) -> None:
        ...
        if centre_proj and not drew_sprite:
            # hide avatar marker when game over
            if not self.player.fallen:
...
class TightropeGame:
    ...
    def __init__(
        ...
        self.high_score = 0.0
        self.game_over_buttons: dict[str, pygame.Rect] = {}
        self._game_over_logo: Optional[pygame.Surface] = None
        self._game_over_logo_key: Optional[tuple[int, int]] = None
    ...
    def _load_menu_button_skins(self) -> None:
        ...
        mapping = {
            ...
            "menu": "Menu Button.png",
            "new_game": "New game Button.png",
            ...
            if self.waiting_for_start:
                ...
                if self.challenge_active:
                    ...
                    if self.player.fallen:
                        self.high_score = max(self.high_score, self.player.forward)
                        self.game_over = True
            else:
                self.camera.update(dt, self.player)
    ...
    def _draw_game_over(self) -> None:
        ...
        # Game over logo
        logo_path = Path(__file__).resolve().parent.parent / "assets" / "Game Over.png"
        if logo_path.exists():
            target_h = 200
            key = (target_h, width)
            if self._game_over_logo is None or self._game_over_logo_key != key:
                try:
                    logo = pygame.image.load(str(logo_path)).convert_alpha()
                    scale = target_h / logo.get_height()
                    target_w = int(logo.get_width() * scale)
                    self._game_over_logo = pygame.transform.smoothscale(logo, (target_w, target_h))
                    self._game_over_logo_key = key
                except Exception:
                    self._game_over_logo = None
            if self._game_over_logo:
                rect = self._game_over_logo.get_rect(center=(width // 2, height // 2 - 140))
                self.screen.blit(self._game_over_logo, rect)

        # Scores
        score = self.player.forward
        self.high_score = max(self.high_score, score)
        score_text = self.renderer.large_font.render(f"Current Score: {score:05.1f}", True, (240, 240, 240))
        high_text = self.renderer.font.render(f"Highest Score Recorded: {self.high_score:05.1f}", True, (230, 230, 230))
        self.screen.blit(score_text, score_text.get_rect(center=(width // 2, height // 2 - 10)))
        self.screen.blit(high_text, high_text.get_rect(center=(width // 2, height // 2 + 30)))

        # Buttons
        self.game_over_buttons = {}
        button_y = height // 2 + 140
        spacing = 240
        new_game_surf, new_game_rect = self._render_menu_button("New Game", False, target_size=(200, 70), skin_override="new_game")
        new_game_rect.center = (width // 2 - spacing // 2, button_y)
        self.game_over_buttons["new_game"] = new_game_rect
        self.screen.blit(new_game_surf, new_game_rect)

        menu_surf, menu_rect = self._render_menu_button("Menu", False, target_size=(200, 70), skin_override="menu")
        menu_rect.center = (width // 2 + spacing // 2, button_y)
        self.game_over_buttons["menu"] = menu_rect
        self.screen.blit(menu_surf, menu_rect)

        hint = " "
        hint_surf = self.renderer.font.render(hint, True, (220, 220, 220))
    ...
    def _handle_game_over_click(self, pos: tuple[int, int]) -> None:
