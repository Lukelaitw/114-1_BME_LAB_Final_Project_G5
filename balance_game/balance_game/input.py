...
class SocketInput(InputProvider):
    ...
    def poll(self, dt: float) -> InputState:
        ...
        if fresh:
            lean = state.lean
            # jump_latch 機制優先：確保即時反應
            # 如果 jump_latch > 0，則 jump=True（即時觸發）
            # 如果 jump_latch == 0，則回退到 state.jump 或 base_state.jump
            if jump_latch > 0.0:
                jump = True
            else:
                # jump_latch 已歸零，使用 state.jump（通常是 False）或 base_state.jump
    ...
    def _process_line(self, raw: bytes) -> None:
        ...
        if isinstance(payload, dict):
            # 支援 binary lean：專注/放鬆
            if "lean" in payload:
                lean_val = payload["lean"]
                try:
                    # 先嘗試作為數值處理（向後兼容）
                    lean = float(lean_val)
                    lean = max(-1.0, min(1.0, lean))
                except (TypeError, ValueError):
                    # 如果不是數值，嘗試作為 binary 狀態處理
                    lean_str = str(lean_val).lower().strip()
                    if lean_str in ("focus", "專注", "1", "true", "right"):
                        lean = 0.5  # 專注 = 向右傾斜
                    elif lean_str in ("relax", "放鬆", "-1", "false", "left"):
                        lean = -0.5  # 放鬆 = 向左傾斜
                    elif lean_str in ("neutral", "中性", "0", "none"):
                        lean = 0.0
                    # 如果都不匹配，lean 保持 None
            if "jump" in payload:
        ...
        with self._lock:
            ...
            if jump is None:
                ...
                # jump_latch 機制：確保即時反應
                if jump:
                    # 收到 jump=true 時，設置 latch，並將 state.jump 設為 False
                    # 這樣 jump 狀態完全由 jump_latch 控制，可以即時停止
                    self._jump_latch = max(self._jump_latch, 1)
                    jump = False  # 不設置 state.jump=True，讓 jump_latch 完全控制
                else:
                    # 收到 jump=false 時，清除 latch 並設置 state.jump=False
                    self._jump_latch = 0.0
