...
# 注意：已強制使用 CPU，不需要設置 CUDA 環境變量
# 如果未來需要使用 CUDA，可以取消註釋以下兩行並修改 device 設置
    ...
    def __init__(self, model_dir, dataset_type='C', 
        ...
        for i in range(1, n_models + 1):
            ...
            try:
                ...
                loaded_model = torch.load(model_path, map_location='cpu', weights_only=False)
                if isinstance(loaded_model, torch.nn.Module):
                    # 載入的是完整模型，先移到 CPU 再移到目標設備
                    model = loaded_model.cpu().to(self.device)
                else:
                    # 載入的是 state_dict，需要先創建模型再載入
